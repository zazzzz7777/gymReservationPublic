			...
			<div class="info_view">
				<div class="title_view">
					<div class="detail_title">
						<span>{{detail.qicaimingcheng}}</span>
					</div>
				...
				<div class="info_item">
					<div class="info_label">器材分类</div>
					<div  class="info_text" >{{detail.qicaifenlei}}</div>
				</div>
				<div class="info_item">
				...
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth('yundongqicai','租借')" @click="qicaizujieonAcross('租借','','','','')" type="warning">租借</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('yundongqicai','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('yundongqicai','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
		<div class="detail-box">
			<div class="title"><span>注意事项</span></div>
			<div class="content" v-html="detail.zhuyishixiang"></div>
		</div>
...
<script setup>
	...
	const getDetail = () => {
		context?.$http({
            ...
            title.value = res.data.data.qicaimingcheng
	...
	const qicaizujieonAcross = async (btnType,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
			context?.$toolUtil.message('请登录后再操作！','error')
			return false
		}
		if(!btnAuth('yundongqicai',btnType)){
		...
		nextTick(()=>{
			router.push(`/index/qicaizujieAdd?type=cross&&id=${detail.value.id}`)
		})
	}
	onMounted(()=>{
		init()
